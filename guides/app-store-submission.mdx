...
> Publish with `release run`, then use `submit` for lifecycle checks

Automate the complete App Store submission process with the canonical release
pipeline, then use the lower-level submission lifecycle commands to inspect or
cancel review state when needed.
...
<Steps>
  ...
  <Step title="Run the release pipeline">
    ...
    asc release run \
      ...
      --version "1.0.0" \
      --build "BUILD_ID" \
      --metadata-dir "./metadata/version/1.0.0" \
      --confirm
    ```
...
```bash  theme={null}
asc submit preflight --app "YOUR_APP_ID" --version "1.0.0"
```
...
# 4. Run the canonical release pipeline
...
4. Resubmit with the canonical release pipeline:
