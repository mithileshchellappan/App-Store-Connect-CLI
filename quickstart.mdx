...
<Note>
  The canonical App Store publish path is `asc release run ... --confirm`. Use `asc submit preflight`, `asc submit status`, or `asc submit cancel` for lower-level submission lifecycle work.
</Note>
